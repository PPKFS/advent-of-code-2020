...
parseLine :: [Text] -> Maybe Password
parseLine [ints, char, pw] = do
       let (sp1, sp2) = T.breakOn "-" ints
       sp1int <- readMaybe (T.unpack sp1)
       sp2int <- readMaybe  (T.unpack  $ T.drop 1 sp2)
       return $ Password sp1int sp2int (T.head char) pw 
parseLine _ = Nothing

data Password = Password Int Int Char Text deriving Show
doDay2 :: (Password -> Bool) -> IO ()
doDay2 pwcheck = do
       inputs <- lines <$> readFileText "day2.txt"
       print $ length $ filter (maybe False pwcheck) $ map (parseLine . words) inputs

validPWPart1 :: Password -> Bool
validPWPart1 (Password lo hi l pw) = lo <= c && hi >= c where
       c = T.count (T.singleton l) pw

validPWPart2 :: Password -> Bool
validPWPart2 (Password lo hi l pw) = xor (T.index pw (lo-1) == l) (T.index pw (hi-1) == l)